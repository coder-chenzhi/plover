...
public class TypeBasedPointsToAnalysis implements PointsToAnalysis {
	public static final Logger LOGGER = LoggerFactory.getLogger(TypeBasedPointsToAnalysis.class);
	...
	private TypeBasedPointsToAnalysis(boolean allReachable){    	
        if(allReachable){
    		...
    		//LOGGER.info(">> jimplify all classes in {}", Utils.getTimeConsumed(startTime,endTime));
        	...
        	if(!Scene.v().hasCallGraph() && !Scene.v().hasPointsToAnalysis()){
        	    ...
        	    Scene.v().setCallGraph(cg.getCallGraph());
        	    Date endTime=new Date();	
        	    LOGGER.info(">> jimplify reachable classes and build CHA call graph in {}",
    ...
    /** 
     * @param node assume node should be a concrete type representing an abstract object.
